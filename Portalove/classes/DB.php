...
class DB
{
    ...
    public function getPlazyItems()
    {
        $sql = "SELECT * FROM plazy ORDER BY id";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }

    public function getCicavceItems()
    {
        $sql = "SELECT * FROM cicavce ORDER BY id";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }

    public function getVtakyItems()
    {
        $sql = "SELECT * FROM vtaky ORDER BY id";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }
    ...
    public function getOnasItems()
    {
        $sql = "SELECT * FROM onas ORDER BY id";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }
